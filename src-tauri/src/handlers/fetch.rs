...
pub async fn check_session(client: State<'_, Client>) -> Result<ActionResult, ActionResult> {
    let url = "http://localhost:8001/api/v1/auth/session";
...
pub async fn login( client: State<'_, Client>, payload: LoginRequest) -> Result<ActionResult, ActionResult> {
    let url = "http://localhost:8001/api/v1/auth/login";
...
#[tauri::command]
pub async fn logout( client: State<'_, Client>) -> Result<ActionResult, ActionResult> {
    let url = "http://localhost:8001/api/v1/auth/logout";

    let response: Result<reqwest::Response, reqwest::Error> = client.post(url).json(&{}).send().await;
